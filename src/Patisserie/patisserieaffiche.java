 ...
 * @author escobar
 */
public class patisserieaffiche extends BasePatisserie {
    ...
    public patisserieaffiche(Resources res) throws IOException {

        super("Promotion", BoxLayout.y());
        theme = UIManager.initFirstTheme("/theme");
        LinePromo po = new LinePromo();
        Date date = new Date();
        this.res = res;
...
//hne chnbdl kol chy 
        ...
        RadioButton all = RadioButton.createToggle("Promotion", barGroup);
...
//            int check = (int) (po.getDateFin().getTime()-date.getTime());
        ...
        List<Produit> p = ps.findPromoByUser();
        for (Produit pr : p) {
            Image image = URLImage.createToStorage(enc, pr.getImageprod(), "http://localhost/final/web/public/uploads/brochures/Produit/" + pr.getImageprod());
            addButton(image, pr.getNomProd(), pr.getPrixProd(), pr.getNvPrix());
        }
    }
    ...
    private void addButton(Image img, String title, int p, int pr) {
