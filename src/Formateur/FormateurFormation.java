 ...
 */
public class FormateurFormation extends BaseFormFormateur{
    ...
    public FormateurFormation(Resources res) {
        ...
        RadioButton Formation = RadioButton.createToggle("Mes formations", barGroup);
        ...
        Formation.addActionListener(e -> {
            ...
            List<Formation> listF = serviceformation.AfficherListFormationsByIdFormateur();
            try {
            ...
            for (Formation f : listF) {
                ...
                addButtonFormation(f.getIdFor(), image, f.getNomFor(), f.getDateFor(), f);
                
            }

        });
        RadioButton messessions = RadioButton.createToggle("Mes sessions", barGroup);
        messessions.setUIID("SelectBar");
        messessions.addActionListener(e -> {

            cntForm.removeAll();
            cntForm = new Container(BoxLayout.y());
            add(cntForm);
            SessionServices sessionencours = new SessionServices();
            List<Session> listS = sessionencours.AfficherLesSessionsByIdFormateur();
            try {
                enc = EncodedImage.create("/giphy.gif");
            } catch (IOException ex) {
                ex.printStackTrace();
        ...
        add(LayeredLayout.encloseIn(
                GridLayout.encloseIn(2, Formation, messessions),
        ...
        bindButtonSelection(messessions, arrow);
    ...
    private void addButtonFormation(int idfor, Image img, String title, String dateFor, Formation formation) {
        ...
        imageFor.addActionListener(e -> {
            ...
            new SignleSessionFormateur(res, formation).show();
        });
